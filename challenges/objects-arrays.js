...
function compare(a, b) {
  const universityA = a.university.toUpperCase();
  const universityB = b.university.toUpperCase();

  let comparison = 0;
  if (universityA > universityB) {
    comparison = 1;
  } else if (universityA < universityB) {
    comparison = -1;
  }
  return comparison;
}

universities.push(graduates.sort(compare));
...
zooAnimals.forEach(element => {
  str = `Name: ${element.animal_name} Scientific: ${element.scientific_name}`;
  displayNames.push(str);
});
...
const popCount = [];
zooAnimals.forEach(element => {
  popCount.push(element.population);
});
const reducer = (accumulator, currentValue) => accumulator + currentValue;
populationTotal = popCount.reduce(reducer);
